...
//header file for ir switch
#include "lino_msgs/IRswitch.h"
//header file for ultrasonics
#include "lino_msgs/Ultrasonic.h"
...
NewPing sonar[SONAR_NUM] = {
  NewPing(LEFT_TRIGGER, LEFT_ECHO, MAX_DISTANCE),    // Each sensor's trigger pin, echo pin, and max distance to ping. 
  NewPing(CENTER_TRIGGER, CENTER_ECHO, MAX_DISTANCE), 
  NewPing(RIGHT_TRIGGER, RIGHT_ECHO, MAX_DISTANCE)};
...
lino_msgs::IRswitch ir_switch_msg;
ros::Publisher ir_switch_pub("ir_switch", &ir_switch_msg);

lino_msgs::Velocities raw_vel_msg;
ros::Publisher raw_vel_pub("raw_vel", &raw_vel_msg);

lino_msgs::Ultrasonic ping_msg;
ros::Publisher ping_pub("ultrasonics", &ping_msg);
...
void setup()
{
    ...
    if(PUBLISH_IRSWITCH)nh.advertise(ir_switch_pub);
    if(PUBLISH_IMU) nh.advertise(raw_imu_pub);
    if(PUBLISH_ULTRASONICS) nh.advertise(ping_pub);
...
void loop()
{
    ...
    static unsigned long prev_irswitch_time = 0;
    static unsigned long prev_ultrasonics_time = 0;
    ...
    if (PUBLISH_IMU && (millis() - prev_imu_time) >= (1000 / IMU_PUBLISH_RATE))
    {
    ...
    if (PUBLISH_IRSWITCH && (millis() - prev_irswitch_time) >= (1000 / IR_SWITCH_PUBLISH_RATE)){
    publishIRswitch();
    prev_irswitch_time = millis();
    }

    if (PUBLISH_ULTRASONICS && (millis() - prev_ultrasonics_time) >= (1000 / ULTRASONICS_PUBLISH_RATE)){
      publishUltrasonics();
      prev_ultrasonics_time = millis();
    }
...
void moveBase()
{
    ...
    motor1_controller.spin(motor1_pid.compute(req_rpm.motor1, current_rpm1));//req_rpm.motor1
...
void publishIRswitch(){
   ...
   ir_switch_msg.S0 = sensor_val[0];
   ir_switch_msg.S1 = sensor_val[1];
   ir_switch_msg.S2 = sensor_val[2];
   ir_switch_msg.S3 = sensor_val[3];
   ir_switch_msg.S4 = sensor_val[4];
   ir_switch_msg.S5 = sensor_val[5];
   
   ir_switch_pub.publish(&ir_switch_msg);
   //char buffer[50];
   //sprintf (buffer, "%d %d %d %d %d %d", sensor_val[0], sensor_val[1], sensor_val[2], sensor_val[3], sensor_val[4], sensor_val[5]);
   //nh.loginfo(buffer);
}

void publishUltrasonics(){
   ping_msg.left = sonar[0].ping_cm();
   ping_msg.center = sonar[1].ping_cm();
   ping_msg.right = sonar[2].ping_cm();
