    ...
    heading_(0)
{
    ...
    pose_publisher_ = nh_.advertise<geometry_msgs::Pose>("raw_pose", 50);
}
...
void LinoBase::velCallback(const lino_msgs::Velocities& vel)
{
    ...
    //robot's position in x,y, and z
    pose.position.x = x_pos_;
    pose.position.y = y_pos_;
    pose.position.z = 0;
    //robot's heading in quaternion
    pose.orientation.x = odom_quat.x();
    pose.orientation.y = odom_quat.y();
    pose.orientation.z = odom_quat.z();
    pose.orientation.w = odom_quat.w();
